...
st.title("Output")
st.subheader(f"Sales: ${y_pred}")
st.text(f"y[sales]={lr.intercept_.round(2)}+x[promote]({lr.coef_[0].round(2)})")
st.text(f"{y_pred}={lr.intercept_.round(2)}+{promote}({lr.coef_[0].round(2)})")
df = get_data()
